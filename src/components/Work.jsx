                ...
                {elems.map((item,index) =>{
                  return <div key={index} className='elem w-full sm:w-[48%]  mt-10 '>
                   <div className='videos w-full h-[104vw] sm:h-[50vw] overflow-hidden relative'>
                       ...
                       data-scroll 
                       data-scroll-speed='-.5'
                       className='hidden sm:absolute sm:z-[2] sm:top-0 sm:left-0 sm:block w-full h-full object-cover' src={item.img} alt="" />
                      <video className='block  sm:z-[1]  absolute w-full h-full top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 scale-[1.3] ' muted loop autoPlay  src={item.video}></video>
                   </div>
                   <div className='mt-4'>
